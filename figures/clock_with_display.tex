...
\begin{tikzpicture}[oriented WD, bbx=1.2cm, bby=2ex, bb min width=2cm, bb port length=4pt, bb port sep=1.5]
    ...
    \coordinate (midflatR) at (0.9,0);   % right end of flat segment
    \coordinate (midflatL) at (-0.9,0);  % left end of flat segment
    \draw
      (clock.east)
        .. controls ($(clock.east)+(1,0)$) and (midflatR) ..
      (midflatR)
        .. controls (midflatR) and (midflatL) ..
      (midflatL)
        .. controls (midflatL) and ($(meridiem.west)+(-1,0)$) ..
      (meridiem.west);
